  ...
  const checkConflicts = () => {
    ...
    allTimes.forEach((obj) => {
      if (obj.location === filterNewLocation) {
        if (
          daysFiltered.includes(obj.days.split('').sort()[0]) ||
          daysFiltered.includes(obj.days.split('').sort()[1]) ||
          daysFiltered.includes(obj.days.split('').sort()[2])
        ) {
          if (
            obj.startTime < ourNewAddedTime.startTime &&
            obj.endTime < ourNewAddedTime.startTime
          ) {
          } else if (
            obj.startTime > ourNewAddedTime.endTime &&
            obj.endTime > ourNewAddedTime.endTime
          ) {
          } else {
